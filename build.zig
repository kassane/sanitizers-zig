...
pub fn build(b: *std.Build) void {
    ...
    const build_asan = b.option(bool, "asan", "Build Address-Sanitizer") orelse false;
    const build_common = b.option(bool, "san-common", "Build common-sanitizer") orelse false;
    _ = build_common; // autofix
    const build_msan = b.option(bool, "msan", "Build Memory-Sanitizer") orelse false;
    _ = build_msan; // autofix
    const build_lsan = b.option(bool, "lsan", "Build Leak-Sanitizer") orelse false;
    _ = build_lsan; // autofix
    const build_rtsan = b.option(bool, "rtsan", "Build Realtime-Sanitizer") orelse false;
    _ = build_rtsan; // autofix
    // const build_tsan = b.option(bool, "tsan", "Build Thread-Sanitizer") orelse false;
    // const build_ubsan = b.option(bool, "ubsan", "Build UndefinedBehavior-Sanitizer") orelse false;

    const compiler_rt = b.dependency("compiler-rt", .{});

    const lib = if (build_asan)
        buildASan(b, .{
            .target = target,
            .optimize = optimize,
            .dependency = compiler_rt,
        })
    else
        buildRTSan(b, .{
            .target = target,
...
fn buildASan(b: *std.Build, options: struct {
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.Mode,
    dependency: ?*std.Build.Dependency = null,
}) *std.Build.Step.Compile {
    const dep = options.dependency orelse @panic("compiler-rt dependency not provided");
    ...
    lib.addCSourceFiles(.{
        ...
        .files = &.{
            ...
            // "tests/asan_benchmarks_test.cpp",
            // "tests/asan_exceptions_test.cpp",
            // "tests/asan_fake_stack_test.cpp",
            // "tests/asan_globals_test.cpp",
            // "tests/asan_interface_test.cpp",
            // "tests/asan_internal_interface_test.cpp",
            // "tests/asan_mac_test.cpp",
            // "tests/asan_mem_test.cpp",
            // "tests/asan_noinst_test.cpp",
            // "tests/asan_oob_test.cpp",
            // "tests/asan_racy_double_free_test.cpp",
            // "tests/asan_str_test.cpp",
            // "tests/asan_test.cpp",
            // "tests/asan_test_main.cpp",
        },
    });
    lib.addCSourceFiles(.{
        .root = dep.path("lib/asan"),
    ...
    buildLSan(lib, options.dependency);
    buildUBSan(lib, options.dependency);
    buildInterception(lib, options.dependency);
    buildSanCommon(lib, options.dependency);
...
fn buildLSan(lib: *std.Build.Step.Compile, dependency: ?*std.Build.Dependency) void {
...
fn buildRTSan(b: *std.Build, options: struct {
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.Mode,
    dependency: ?*std.Build.Dependency = null,
}) *std.Build.Step.Compile {
    ...
    lib.addCSourceFiles(.{
        ...
        .files = &.{
            ...
            // "tests/rtsan_test_functional.cpp",
            // "tests/rtsan_test_interceptors.cpp",
            // "tests/rtsan_test_main.cpp",
        },
        .flags = cxxflags,
    });
    buildInterception(lib, options.dependency);
